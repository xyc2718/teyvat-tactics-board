...
- **基础模式**：可为双方球员选择水、火、冰、雷、岩、风，调整初始站位并绘制静态移动箭头；除风外可查看攻击范围与“Q + 攻击”打击范围，不创建时间轴动作。
- **推演模式**：支持水、火、冰、岩、雷五种职业，编排跑动、等待、Q 技能、雷 E、传球、空传、射门、说明、挂冰和冰圈，并在统一时间轴上播放。
...
- **技能与状态**：区分五职业的 Q 距离、持续时间、冷却和后续效果；支持雷 E 能量与冲刺、冻结、挂冰减速、随霜役移动的冰圈，以及水 Q 和接球加速路径分段。
...
普通定向传球的新默认值是 **10 格、最长 3 秒**，速度随时间线性减小，接到球则提前结束。**已有战术、草稿、历史记录及导入文件保留原参数**，在旧战术中新增传球仍按其保存的参数计算；只有新建战术或主动恢复默认规则才使用新值。空传仍为 6 格、3 秒。

绘制普通传球时，球场显示 **4 / 8 / 10 格**三道距离圈；4–8 格为普通截断警示，8–10 格为更醒目的远距高风险警示，超过最大距离则按落地处理。若战术保存了不同的传球参数，安全圈和最远圈仍按该战术的规则显示；8 格分界仅在两圈之间时出现。

### 基础模式职业选择

切换到“基础模式”，点击球员后，在球场外的职业选择栏选择水、火、冰、雷、岩或风。每名球员的基础职业独立保存，支持撤销、重做和存档；切回推演模式仍使用原来设置的推演职业，不会改写已有动作。两种模式继续共用初始站位。

风目前仅用于职业标识、站位和移动箭头，尚未配置攻击半径或 Q 距离，不参与推演技能计算。雷与岩在基础模式中也可显示攻击/打击范围。

### 雷职业与 E 冲刺

雷的普攻范围暂与水相同，Q 为 **固定 1.8 格**瞬时位移、冷却 **10 秒**。顶部“雷 E”（快捷键 `T`）创建紫色冲刺路线，支持直线、曲线和固定时间。

- 新战术初始满能量；满能量冲刺 **8 格、3.8 秒**，速度恒定约 2.11 格/秒，不受普通跑动加速、挂冰或冰圈减速影响。
- 冲刺中只消耗能量，不回复；未冲刺时均匀回复，空能量到满需要 **15 秒**，冷却和冻结期间也回复。检查器显示当前能量、冷却及可冲刺距离。
- 持球不能开启 E。主动停止、能量耗尽、接球、被冻结或中途使用 Q 都会结束 E，从实际停止时刻开始 **4 秒冷却**；剩余能量保留，不会自动续冲。
- 固定时间决定可移动距离，不会改变速度；超过当前能量预算时会提示。E 开始、结束和冷却结束均可作为时间轴关键帧。
- 队友传球到达时自动中断 E，剩余路线转为**独立普通跑动**，恢复普通加减速规则；可单独编辑或删除，删除后不会自动重建。撤销这次传球会恢复原冲刺。
- 传球路线用独立颜色标记 E 截球威胁；射门受压分析也考虑 E 与 Q + E，并计入当前能量、冷却、冻结和持球限制。分析采用有界的单次 E / Q 组合估算，不自动施放技能或改变球权。

### 万象（岩）职业与护罩分析

万象简称“岩”，默认攻击半径为 **1.5 格**，Q 为 **固定 2.4 格**的瞬时位移、冷却 **8 秒**；遇场地边界会截短。护罩按自身周围 **1 格**计算，仅作为风险分析，不需要添加开盾动作，也不会自动改写球权。
...
上述抵达时间采用基础跑速近似，考虑 Q 固定长度、场地边界、冷却与冻结，不额外求解最优绕路或加减速路径。旧战术的已有参数和对位值会保留，仅为缺少的职业配置补入默认值；可在“规则设置”中查看五职业对位表。
...
- 水、火、岩与雷的 Q 在规则时间中是瞬时动作，同一时刻保留可区分的 Q 起点和 Q 终点；冰 Q 默认持续约 1 秒。
- 定向传球的接球时刻由程序解算，不要求预先落在任何已有关键帧上。
- 新战术的定向传球速度随时间线性减小，最多飞行 3 秒、累计 10 格（按曲线路径长度计算）；若仍未追到接球人，球会停在实际落点，不生成接球帧或接球加速。旧战术使用自身保存的参数。
...
- 推演模式包含水灵、蛮牛、霜役、万象和雷五个职业；基础模式额外支持风的职业标识，不包含风的技能规则或范围参数。
