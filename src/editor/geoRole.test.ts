...
describe('Geo simulation role', () => {
  ...
  it.each(['start', 'end'] as const)('keeps the %s Q edge through an anchored pass, path edit and file round trip', (edge) => {
    ...
    expect(frame.cooldowns[q.actorId]?.q).toBe(edge === 'start' ? 0 : 8)
    state.setTool('pass')
  ...
  it.each([4, 12])('waits for overlapping freeze and cooldown before the next Q (thaw=%s)', (thaw) => {
    ...
    const state = useTacticStore.getState()
    state.setTool('qMove')
    expect(useTacticStore.getState().currentTime).toBe(Math.max(thaw, 8))
    ...
    expect(actions[1]!.startTime).toBe(Math.max(thaw, 8))
    expect(actions[1]!.path[0]).toEqual(actions[0]!.path.at(-1))
    expect(validateQStart(document, 'blue-water', 8).valid).toBe(true)
    expect(validateQStart(document, 'blue-water', 7.99).valid).toBe(false)
    expect(earliestLegalQStart(document, 'blue-water', 1)).toBe(8)
  })
  ...
  it('reflows role and rule edits with undo/redo and resets to canonical Geo defaults', () => {
    ...
    state.undo()
    expect(pathLength(geoQ().path)).toBeCloseTo(3)
    state.updateRoleRule('geo', 'qCooldown', 12)
    state.resetRules()
    expect(pathLength(geoQ().path)).toBeCloseTo(2.4)
    expect(useTacticStore.getState().document.rulesSnapshot.roles.geo.shield?.radius).toBe(1)
    expect(useTacticStore.getState().document.rulesSnapshot.roles.geo.q.cooldown).toBe(8)
    expect(useTacticStore.getState().document.rulesSnapshot.roles.fire.q.cooldown).toBe(9)
    state.resetTactic()
    expect(useTacticStore.getState().document.rulesSnapshot.roles.geo.q.cooldown).toBe(8)
